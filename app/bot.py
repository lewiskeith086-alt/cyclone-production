...
from .services.payment_service import create_payment_invoice
...
async def escape_search_state_to_menu(message: Message, state: FSMContext):
    ...
    if message.text == "📦 Plans":
        await plans_handler(message, state)
    elif message.text == "💰 Top Up":
        await topup_handler(message, state)
    ...
    elif message.text == "🧾 Invoice Status":
        await invoice_status_handler(message, state)
...
async def capture_query_handler(message: Message, state: FSMContext):
    ...
    try:
        ...
        logging.exception("Search failed for telegram_id=%s query=%s", message.from_user.id, query)
        await message.answer("❌ Search failed right now. Please try again in a moment.", parse_mode="HTML")
        await state.clear()
...
async def _run_export(callback: CallbackQuery, state: FSMContext, file_kind: str):
    ...
    try:
        ...
        if file_kind == "txt":
            ...
            suffix = ".txt"
            filename = f"{safe_query_name(query)}_results.txt"
        else:
            content = build_safe_csv(records)
            suffix = ".csv"
            filename = f"{safe_query_name(query)}_results.csv"

        with tempfile.NamedTemporaryFile("w", delete=False, suffix=suffix, encoding="utf-8", newline="") as tmp:
            tmp.write(content)
            temp_path = tmp.name
        ...
        logging.exception("Export failed for telegram_id=%s query=%s", callback.from_user.id, query)
...
async def buy_unlimited_handler(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        charge_wallet(db, user, UNLIMITED_PLAN_CENTS, "unlimited_plan", "Unlimited plan purchase")
        activate_unlimited(db, user, unlimited_expiry())

        await callback.message.answer(
            f"✅ <b>Unlimited plan activated.</b>\n\nActive until: <b>{user.unlimited_until}</b>",
            parse_mode="HTML",
        )
    except Exception:
...
async def topup_handler(message: Message, state: FSMContext):
    await cancel_search_state_if_active(state)
    await state.set_state(PaymentTopUpStates.waiting_for_asset)
    await message.answer(
        "💸 <b>Top Up</b>\n\nChoose payment asset:\n1. BTC\n2. USDT (TRC20)\n\nReply with BTC or USDT",
        parse_mode="HTML",
    )


@router.message(PaymentTopUpStates.waiting_for_asset)
async def payment_asset_handler(message: Message, state: FSMContext):
    asset = (message.text or "").strip().upper()
    if asset not in {"BTC", "USDT"}:
        await message.answer("Reply with BTC or USDT")
        return
    await state.update_data(asset=asset)
    await state.set_state(PaymentTopUpStates.waiting_for_amount)
    await message.answer("Enter amount in USD, e.g. 10 or 25.50")


@router.message(PaymentTopUpStates.waiting_for_amount)
async def payment_amount_handler(message: Message, state: FSMContext):
    raw = (message.text or "").strip().replace("$", "")
    try:
        usd_cents = int(round(float(raw) * 100))
    except ValueError:
        await message.answer("Please enter a valid USD amount.")
        return
...
@router.message(F.text == "🧾 Invoice Status")
async def invoice_status_handler(message: Message, state: FSMContext):
    await cancel_search_state_if_active(state)
    db = get_session()
    try:
        invoice = (
            db.query(PaymentInvoice)
            .filter(PaymentInvoice.telegram_id == message.from_user.id)
        ...
        await message.answer(
            ...
            f"Network: <b>{invoice.network}</b>\n"
            f"USD Amount: <b>${invoice.amount_usd_cents / 100:.2f}</b>\n"
            f"Expected Amount: <b>{invoice.amount_crypto} {invoice.asset}</b>\n"
            f"Address: <code>{invoice.wallet_address}</code>\n"
            f"Status: <b>{invoice.status.upper()}</b>\n"
            f"Confirmations: <b>{invoice.confirmations}</b>\n"
            f"Expires At: <b>{expires_at}</b>\n"
            f"Paid At: <b>{paid_at}</b>",
            parse_mode="HTML",
        )
    finally:
        db.close()
...
@router.message(F.text == "💸 Invoices")
async def admin_invoices_handler(message: Message, state: FSMContext):
    await cancel_search_state_if_active(state)
    ...
    db = get_session()
    try:
        invoices = db.query(PaymentInvoice).order_by(PaymentInvoice.created_at.desc()).limit(10).all()
        if not invoices:
            await message.answer("No invoices found.")
            return

        lines = ["💸 <b>Latest Invoices</b>", ""]
        for inv in invoices:
            lines.append(
                f"• <code>{inv.invoice_id}</code>\n"
                f"  tg: <code>{inv.telegram_id}</code>\n"
                f"  {inv.asset}/{inv.network} | ${inv.amount_usd_cents / 100:.2f}\n"
                f"  expected: {inv.amount_crypto}\n"
                f"  status: <b>{inv.status}</b>\n"
                f"  confs: {inv.confirmations}\n"
            )
...
async def admin_stats_handler(message: Message, state: FSMContext):
    ...
    try:
        ...
        total_invoices = db.query(func.count(PaymentInvoice.id)).scalar() or 0
...
async def broadcast_send_handler(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        for target in users:
            try:
                await callback.bot.send_message(chat_id=target.telegram_id, text=f"📢 Broadcast\n\n{text}")
                sent += 1
            except Exception:
                failed += 1
...
async def add_balance_apply_handler(callback: CallbackQuery, state: FSMContext):
    ...
    db = get_session()
    try:
        user = add_wallet_balance(db, int(data["target_telegram_id"]), int(data["amount_cents"]), "admin_manual_topup")
        if not user:
            await callback.message.answer("User not found.")
        else:
            await callback.message.answer(
