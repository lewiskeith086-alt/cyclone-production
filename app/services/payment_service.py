...
def get_receive_address(asset: str) -> tuple[str, str]:
    ...
    if asset == "BTC":
        if not BTC_RECEIVE_ADDRESS:
            raise RuntimeError("BTC_RECEIVE_ADDRESS is missing")
        return BTC_RECEIVE_ADDRESS, "BTC"

    if asset == "USDT":
        if not USDT_TRC20_RECEIVE_ADDRESS:
            raise RuntimeError("USDT_TRC20_RECEIVE_ADDRESS is missing")
...
def create_payment_invoice(db: Session, telegram_id: int, asset: str, usd_cents: int) -> PaymentInvoice:
    ...
    unique_bump = Decimal(str((uuid.uuid4().int % 97) + 1)) / (
        Decimal("100000000") if asset == "BTC" else Decimal("1000000")
    )
